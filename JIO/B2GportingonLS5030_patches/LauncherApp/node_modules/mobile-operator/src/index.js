var MobileOperator = {
  ...
  userFacingInfo: function mo_userFacingInfo(mobileConnection) {
    ...
    if (this.simCardsName && this.simCardsName[iccid]) {
      operator = this.simCardsName[iccid];
    }
  ...
  getMobileInfo: function mo_getMobileInfo() {
    return this.MOBILE_INFO;
  },

  start: function mo_start() {
    var settings = window.navigator.mozSettings;
    if (settings) {
      settings.createLock().get('custom.simcards.name').then((value) => {
        this.simCardsName = value['custom.simcards.name'];
      });
      settings.addObserver('custom.simcards.name', (event) => {
        this.simCardsName = event.settingValue;
      });
    }
  }
};

MobileOperator.start();
