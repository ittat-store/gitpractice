...
extern int ask_callback(struct fdisk_context *cxt, struct fdisk_ask *ask, void *data __attribute__((__unused__)));
...
int main(int argc, char **argv)
{
	...
	/*while ((c = getopt_long(argc, argv, "b:Bc::C:hH:lL::o:sS:t:u::vVw:W:",
		...
		case 'c':
			if (optarg) 
			{
				// this setting is independent on the current actively used label
	...
	if (argc-optind != 1 && fdisk_has_user_device_properties(cxt))
		warnx(_("The device properties (sector size and geometry) should be used with one specified device only."));
	...
	case ACT_FDISK:
		...
		if (!fdisk_has_label(cxt)) 
		{
			...
			fdisk_warnx(cxt, _("A hybrid GPT was detected. You have to sync the hybrid MBR manually (expert command 'M')."));
